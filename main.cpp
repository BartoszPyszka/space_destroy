...
public:
    ...
    // Aktualizacja stanu gracza (obrót)
...
private:
    ...
    float angle;               // Kąt obrotu gracza
    sf::VertexArray shape;     // Kształt gracza (graficzna reprezentacja)
};

// Konstruktor gracza — ustawienie pozycji, kąta i kształtu
Player::Player()
    : position(500.f, 500.f), angle(45.f), shape(sf::LineStrip, 5)
{
    ...
    shape[4].position = shape[0].position; // Zamknięcie kształtu

    // Ustawienie koloru wszystkich wierzchołków
...
// Aktualizacja gracza — obsługa obrotu
